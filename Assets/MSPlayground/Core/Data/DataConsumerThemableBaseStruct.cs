...
namespace MSPlayground.Core.Data
{
    ...
    ///
    /// Generic type T is the type of object expected from the data source, where T is a struct
    /// Generic type U is the Component type in the scene hierarchy where that object of type T will be modified.
    ///
    /// NOTE: This class is based off of and an extension of {Microsoft.MixedReality.Toolkit.Data.DataConsumerThemableBase<T>}
    /// where T is a class. This extension allows us to theme values that are structs. It has lesser retrieval methods than
    /// DataConsumerThemableBase<T>. Explicitly, DataConsumerThemableBaseStruct<T> does not support retrieval via resource path or
    /// streaming asset path. If trying to use these data retrieval methods, it will just return default of T.
    ///
    /// </summary>
    /// <typeparam name="T">The type of the object expected (where type is a struct). E.g. Color</typeparam>
    /// <typeparam name="U">The type of Component to be managed, typically a Renderer, Material, or Texture2D</typeparam>
    [Serializable]
    public abstract class DataConsumerThemableBaseStruct<T> : DataConsumerThemableBase where T : struct
    {
        ...
        /// DataConsumerThemeHelper.  Whatever value is stored in the matching field of the
        ...
        protected virtual T GetObjectByThemeLookupIndex(BindingInfo binding, int n)
        {
            ...
            {
                return (T)binding.ThemeSource.GetValue(binding.BindingProfile.ValueToThemeKeypathLookup[n].Keypath);
            }
            else
            {
                return default;
        ...
        protected virtual T GetObjectByThemeLookupKey(BindingInfo binding, string keyValue)
        {
            ...
            foreach (DataBindingProfile.ValueToKeypath valueToKeypath in binding.BindingProfile
                         .ValueToThemeKeypathLookup)
            {
                if (keyValue == valueToKeypath.Value)
                {
                    return (T)binding.ThemeSource.GetValue(valueToKeypath.Keypath);
                }
            }
        ...
        private void AddComponentToBindingInfoList(int whichSlot, DataBindingProfile bindingProfile,
            Component component)
        {
            BindingInfo binding;
        ...
        protected override void ProcessDataChanged(IDataSource dataSource, string resolvedKeyPath, string localKeyPath,
            object inDataOrThemeValue, DataChangeType dataChangeType)
        {
        ...
        protected virtual void ProcessThemeOrDataChange(BindingInfo binding, object dataValue, object themeValue = null)
        {
            ...
            switch (binding.BindingProfile.RetrievalMethod)
            {
                ...
                case DataRetrievalMethod.DirectValue:
                    outputValue = (T)dataValue;
                    break;
                case DataRetrievalMethod.FilePath:
                    Debug.LogWarning($"[{this.GetType().ToString()}] DataConsumerThemableBaseStruct does not support data retrieval via" +
                                     $"file path. Returning default.");
                    outputValue = default;
                    break;
                case DataRetrievalMethod.ResourcePath:
                    Debug.LogWarning($"[{this.GetType().ToString()}] DataConsumerThemableBaseStruct does not support data retrieval via" +
                                     $"resource path. Returning default.");
                    outputValue = default;
                    break;
        ...
        {
            return binding.BindingProfile.ValueToThemeKeypathLookup != null &&
                   binding.BindingProfile.ValueToThemeKeypathLookup.Length > 0;
        }
